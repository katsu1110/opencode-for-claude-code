---
description: Perform a cross-model first-pass review of code changes via OpenCode Go
argument-hint: "[--adversarial] [scope: paths or git range]"
---

Use OpenCode Go as an **independent, different-model reviewer** of the current
changes, then reconcile the findings yourself (you are the final judge).
...
2. Delegate the review to opencode (pro tier) — pipe the diff in on stdin:
   `git diff | oc-delegate --tier pro -`
   with an instruction to find correctness/security/performance bugs, be skeptical, and
   list each as `file:line — issue`. If `--adversarial` is set, also have it challenge
   the design decisions and tradeoffs, not just line bugs.
