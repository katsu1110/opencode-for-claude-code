...
2. **Fan-out search (opencode, cheap, one call per sub-question).** Web tools need
   `--write` (opencode's `--auto` grants permissions):
   `oc-delegate --tier flash --write "Use web search for <sub-question>. Return 5-8 bullet findings, each with the exact source URL. Output ONLY findings + URLs."`
