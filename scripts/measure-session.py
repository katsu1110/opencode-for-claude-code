...
and tool-call counts. NOTE: this measures the CLAUDE side only — opencode Go tokens
are not exposed by `opencode run` in headless mode, so the cheap-side cost must be
priced separately via oc-cost-compare.
"""
