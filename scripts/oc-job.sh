#!/usr/bin/env bash
#
# oc-job.sh — background-job layer over oc-delegate.sh.
...
#
# Usage:
#   oc-job.sh start  [oc-delegate options] "task"   # -> prints a JOB_ID, returns now
#   oc-job.sh list                                    # jobs started from this dir
#   oc-job.sh status <id>                             # running | done(rc) | failed
#   oc-job.sh result <id>                             # print stdout (+rc) when finished
#   oc-job.sh cancel <id>                             # terminate a running job
#
# Jobs live under ${OPENCODE_JOBS:-~/.opencode-jobs}/<id>/ (out, err, rc, meta).
#
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
DELEGATE="${OC_DELEGATE:-$HERE/oc-delegate.sh}"
REG="${OPENCODE_JOBS:-$HOME/.opencode-jobs}"

die() { echo "oc-job: $*" >&2; exit 1; }
...
rc_label() {
  case "$1" in
    ...
    11) echo 'AUTH required — run `opencode auth login` interactively' ;;
    12) echo 'TIMEOUT — raise --timeout or narrow scope' ;;
    13) echo 'opencode MISSING — install the OpenCode CLI' ;;
    14) echo 'MODEL unavailable — check `opencode models`' ;;
    *)  echo 'error' ;;
  esac
}
...
case "$cmd" in
  ...
  status)
    ...
    sig="$(grep -m1 '^OC_SIGNAL ' "$jd/err" 2>/dev/null || true)"
    if [ -n "$sig" ]; then echo "  signal=${sig#OC_SIGNAL }"; fi
    ;;
  result)
    jd="$(jobdir "${1:-}")"; st="$(job_state "$jd")"
