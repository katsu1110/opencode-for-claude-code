...
# (oc-diagnose / oc-delegate / oc-delegate.sh / oc-job / oc-job.sh /
#  oc-cost-compare); block everything else with exit code 2.
#
...
# component is exactly one of the allowed wrappers — a lookalike such as
...
DELEG = r"(?:[\w./~+:@-]*/)?(?:oc-diagnose|oc-delegate|oc-job|oc-cost-compare)(?:\.sh)?"
WORDS = r"[ \t\w./~+:@=,%-]*"
...
echo "[opencode-delegate] blocked: this subagent may only run oc-diagnose / oc-delegate / oc-job / oc-cost-compare via Bash (bare name or unquoted path; single command, no chaining/substitution; pipe literal prompts with echo/printf | oc-delegate -). Delegate file work to opencode via --dir; verification is the caller's job." >&2
exit 2
